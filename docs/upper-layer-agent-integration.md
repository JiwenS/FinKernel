...
- `POST /api/profiles/discovery/drafts/{draft_id}/confirm-profile`
...
`POST /api/profiles/assess-profile` as the single orchestration checkpoint
before each discovery-state refresh, draft, or update prompt.

Legacy persona-named routes remain available for compatibility.
